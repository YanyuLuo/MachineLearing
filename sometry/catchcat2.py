...
class tm(object):  ####手机端
    ...
    def goodsid(self, url):  ###通过店铺URL获取店铺所有ID
        ...
        searchurl = 'https://{}.m.tmall.com/shop/shop_auction_search.do?spm=a1z60.7754813.0.0.301755f0pZ1GjU&sort' \
                    '=defaul'.format(
            shopname)
        s = requests.session()
        headers = {'Accept': '*/*',
                   ...
                   'Referer': 'https://{}.m.tmall.com/shop/shop_auction_search.htm?spm=a1z60.7754813.0.0.301755f0pZ1GjU&sort=default'.format(
                       shopname)
                   }
        ...
        if total_page != 1:
            ...
            for i in range(2, total_page + 1):
                ...
                data = {'shop_id': shop_id_list, 'shop_title': shop_title_list, 'item_id': item_id, 'title': title,
                        'sold': sold, 'totalSoldQuantity': totalSoldQuantity, 'skuurl': skuurl, 'price': price}
                df = pd.DataFrame(data=data)
                df.to_csv(self.path + r'\tmgoodsid{}.csv'.format(shopname), mode='a', index=False, header=0,
                          encoding="GB18030")
    ...
    def getiddata(self, id):  ###获取ID数据
        ...
        headers = {'Accept': '*/*',
                   'Accept-Language': 'zh-CN',
                   'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_3 like Mac OS X) AppleWebKit/603.3.8 (KHTML, like Gecko) FxiOS/10.6b8836 Mobile/14G60 Safari/603.3.8',
                   'Referer': 'https://detail.m.tmall.com/item.htm?spm=a220m.6910245.0.0.55b17434eiwv4f&id={}'.format(
                       id)
                   }
        print(url)
        s.headers.update(headers)
